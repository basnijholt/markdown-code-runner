...
class ProcessingState:
    ...
    def process_line(self, line: str, *, verbose: bool = False) -> None:
        ...
        if is_marker(line, "skip"):
            ...
            self.section = "code:backtick:file"
        ...
        *,
        ...
        if is_marker(line, end_marker):
            if not self.skip_code_block:
                output_file = None
                print(self.section)
                if self.section.endswith(":file"):
                    language = "file"
                    output_file = self.section.split(":file:")[1]
                    assert output_file, "Output file must be specified"
                elif self.section.endswith(":bash"):
                    language = "bash"
                else:
                    language = "python"
